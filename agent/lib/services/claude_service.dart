...
class ClaudeService {
          ...
          try {
            ...
            if (type == 'content_block_delta') {
              ...
              if (delta['type'] == 'text_delta') {
                yield ClaudeStreamChunk(content: delta['text'] ?? '', done: false);
              }
            } else if (type == 'message_stop') {
              // Final event - no more content
              yield ClaudeStreamChunk(content: '', done: true);
            }
          } catch (e) {
  ...
  Future<bool> verifyApiKey() async {
    try {
      final response = await query(sanitizedQuery: 'Respond with "OK" if you can read this.', maxTokens: 10);
...
class ClaudeResponse {
  ...
  ClaudeResponse({required this.content, required this.stopReason, required this.usage});
}
...
class ClaudeStreamChunk {
  ...
  ClaudeStreamChunk({required this.content, required this.done});
}
...
class ClaudeUsage {
  ...
  ClaudeUsage({required this.inputTokens, required this.outputTokens});
